...
struct usb_interface_descriptor usb_interface_kbd = {
    .bLength = USB_DT_INTERFACE_SIZE,
    .bDescriptorType = USB_DT_INTERFACE,
    .bInterfaceNumber = KBD_INTERFACE,
    .bAlternateSetting = 0,
    .bNumEndpoints = 1,
    .bInterfaceClass = USB_CLASS_HID,
    .bInterfaceSubClass = 1,
...
struct usb_interface_descriptor usb_interface_mouse = {
    .bLength = USB_DT_INTERFACE_SIZE,
    .bDescriptorType = USB_DT_INTERFACE,
    ...
    .bInterfaceClass = USB_CLASS_HID,
    .bInterfaceSubClass = 1,
    .bInterfaceProtocol = 2, // mouse = 2 (important)
    .iInterface = STRING_ID_INTERFACE,
};
...
struct usb_endpoint_descriptor usb_ep_kbd = {
    .bLength = USB_DT_ENDPOINT_SIZE,
    .bDescriptorType = USB_DT_ENDPOINT,
    .bEndpointAddress = USB_DIR_IN | 1,   // EP 1 IN
    .bmAttributes = USB_ENDPOINT_XFER_INT,
    .wMaxPacketSize = EP_MAX_PACKET_INT,
    .bInterval = 5,
};

struct usb_endpoint_descriptor usb_ep_mouse = {
    .bLength = USB_DT_ENDPOINT_SIZE,
    .bDescriptorType = USB_DT_ENDPOINT,
...
struct hid_descriptor usb_hid_mouse = {
	.bLength =		9,
	...
	.desc =			{
		{
			...
			.wDescriptorLength =	sizeof(usb_hid_report_mouse),
		}
	},
};

struct hid_descriptor usb_hid_keyboard = {
	.bLength =		9,
	.bDescriptorType =	HID_DT_HID,
	.bcdHID =		__constant_cpu_to_le16(0x0110),
	.bCountryCode =		0,
	...
	.desc =			{
		{
			.bDescriptorType =	HID_DT_REPORT,
			.wDescriptorLength =	sizeof(usb_hid_report_keyboard),
		}
	},
};
int build_config(char *data, int length, bool other_speed) {
    ...
    // 🟢 Keyboard interface
    memcpy(data, &usb_interface_kbd, sizeof(usb_interface_kbd));
    data += sizeof(usb_interface_kbd);
    length -= sizeof(usb_interface_kbd);
    total_length += sizeof(usb_interface_kbd);

    memcpy(data, &usb_hid_keyboard, sizeof(usb_hid_keyboard));
    data += sizeof(usb_hid_keyboard);
    length -= sizeof(usb_hid_keyboard);
    total_length += sizeof(usb_hid_keyboard);

    memcpy(data, &usb_ep_kbd, USB_DT_ENDPOINT_SIZE);
    data += USB_DT_ENDPOINT_SIZE;
    length -= USB_DT_ENDPOINT_SIZE;
    total_length += USB_DT_ENDPOINT_SIZE;

    // 🟢 Mouse interface
    memcpy(data, &usb_interface_mouse, sizeof(usb_interface_mouse));
    data += sizeof(usb_interface_mouse);
    length -= sizeof(usb_interface_mouse);
    total_length += sizeof(usb_interface_mouse);

    memcpy(data, &usb_hid_mouse, sizeof(usb_hid_mouse));
    data += sizeof(usb_hid_mouse);
    length -= sizeof(usb_hid_mouse);
    total_length += sizeof(usb_hid_mouse);

    memcpy(data, &usb_ep_mouse, USB_DT_ENDPOINT_SIZE);
    data += USB_DT_ENDPOINT_SIZE;
    length -= USB_DT_ENDPOINT_SIZE;
    total_length += USB_DT_ENDPOINT_SIZE;
...
int ep_mouse = -1;
int ep_kbd = -1;
pthread_t ep_keyboard_in_thread;
bool ep_keyboard_in_thread_spawned = false;


void *ep_mouse_in_loop(void *arg) {
	int fd = (int)(long)arg;

	// Keyboard sends 8-byte HID reports.
	struct usb_raw_int_io io;
	io.inner.ep = ep_mouse;
	io.inner.flags = 0;
	io.inner.length = 4;
	memset(&io.inner.data[0], 0, 4);

	while (true) {

		int dx = 0, dy = 0, btn = 0, wheel = 0;

		FILE *f = fopen("/tmp/mouse_delta", "r");
		if (f) {
			int n = fscanf(f, "%d %d %d %d", &dx, &dy, &btn, &wheel);
			if (n != 4) {
				dx = dy = btn = wheel = 0;
			}
			fclose(f);
		...
		// clamp to HID int8 range (-127..127)
		if (dx > 127) dx = 127;
		if (dx < -127) dx = -127;
		if (dy > 127) dy = 127;
		if (dy < -127) dy = -127;
		...
		btn &= 0x03; // only left/right supported

		// build HID mouse report
		io.inner.data[0] = (int8_t)btn;
		io.inner.data[1] = (int8_t)dx;
		io.inner.data[2] = (int8_t)dy;
		io.inner.data[3] = (int8_t)wheel;

		int rv = usb_raw_ep_write_may_fail(fd,
				(struct usb_raw_ep_io *)&io);

		if (rv < 0 && errno == ESHUTDOWN) {
			printf("mouse: device reset, exiting\n");
			break;
		} else if (rv < 0) {
		...
		printf("mouse: dx=%d dy=%d (sent %d bytes)\n", dx, dy, rv);

		usleep(5000);  // IMPORTANT: smooth + responsive (~200Hz)
	}
...
void *ep_keyboard_in_loop(void *arg) {
	int fd = (int)(long)arg;
	...
	while (true) {
        ...
        FILE *f = fopen("/tmp/kbd_event", "r");
        if (f) {
            fscanf(f, "%d %d", &mod, &key);
            fclose(f);
        }
		...
		io.inner.data[0] = (uint8_t)mod;   // modifier byte
        io.inner.data[2] = (uint8_t)key;   // keycode

		printf("mod=%d key=%d\n",
			(int8_t)io.inner.data[0],
			(int8_t)io.inner.data[2]);
	...
	case USB_TYPE_STANDARD:
		...
		case USB_REQ_GET_DESCRIPTOR:
			...
			case USB_DT_STRING:
				...
				return true;
			case HID_DT_REPORT:
				if (event->ctrl.wIndex == KBD_INTERFACE) {
					memcpy(io->data, usb_hid_report_keyboard,
						sizeof(usb_hid_report_keyboard));
					io->inner.length = sizeof(usb_hid_report_keyboard);
					return true;
				}

				if (event->ctrl.wIndex == MOUSE_INTERFACE) {
					memcpy(io->data, usb_hid_report_mouse,
						sizeof(usb_hid_report_mouse));
					io->inner.length = sizeof(usb_hid_report_mouse);
					return true;
				}

				printf("unknown interface for HID report\n");
				exit(EXIT_FAILURE);
			default:
		...
		case USB_REQ_SET_CONFIGURATION:
			...
			ep_kbd = usb_raw_ep_enable(fd, &usb_ep_kbd);
			ep_mouse = usb_raw_ep_enable(fd, &usb_ep_mouse);
			printf("ep0: ep_int_in enabled: %d\n", ep_int_in);
			int rv = pthread_create(&ep_int_in_thread, 0,
					ep_mouse_in_loop, (void *)(long)fd);
			if (rv != 0) {
				perror("pthread_create(ep_int_in)");
				exit(EXIT_FAILURE);
			}
			int rv2 = pthread_create(&ep_keyboard_in_thread, 0,
					ep_keyboard_in_loop, (void *)(long)fd);
			if (rv2 != 0) {
				perror("pthread_create(ep_int_in)");
				exit(EXIT_FAILURE);
			}
			...
			// ep_keyboard_in_thread_spawned = true;
...
void ep0_loop(int fd) {
	while (true) {
		...
		if (event.inner.type == USB_RAW_EVENT_RESET) {
			...
			if (ep_keyboard_in_thread_spawned) {
				pthread_cancel(ep_keyboard_in_thread);
				int rv = pthread_join(ep_keyboard_in_thread, NULL);
				if (rv != 0) {
					perror("pthread_join(ep_int_in)");
					exit(EXIT_FAILURE);
				}
				ep_keyboard_in_thread_spawned = false;
